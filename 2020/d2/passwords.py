...
def is_valid(match):
    ...
    if 0 < lower < upper <= len(password):
        return (password[lower - 1] == letter) is not (password[upper - 1] == letter)
...
def main():
    ...
    pattern = re.compile(
        "\A(?P<lower>[0-9]+)-(?P<upper>[0-9]+)\s(?P<letter>[a-z]):\s(?P<password>[a-z]+)\s*\Z"
    )
